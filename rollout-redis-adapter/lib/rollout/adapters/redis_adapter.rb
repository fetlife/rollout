  ...
  module Adapters
    class RedisAdapter
      ...
      def fetch_feature(name)
        Redis::Codec.decode(name, @client.get(key(name)))
      end
      ...
      def fetch_features(names)
        ...
        names.zip(payloads).map { |name, payload| Redis::Codec.decode(name, payload) }
      end
      ...
      def save_feature(state)
        @client.set(key(state.name), Redis::Codec.encode(state))
      ...
      def events_from(storage_key, limit: nil)
        ...
        @client
          ...
          .map { |value| Redis::Codec.decode_event(*value) }
          .reverse
      end
