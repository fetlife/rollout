...
If your product is an international one, you might need this feature.
You can select which locale will receive this rollout with certain proportion.

You can activate all the locale groups for the chat feature:

  $rollout.activate_locale(:chat, :all)

You need to define your own locale groups:

  $rollout.define_locale("zh-CN") { |user| user.language == "zh-CN" }

Roll out to certain percentage of locale looks like this:

  $rollout.activate_locale(:chat, "zh-CN", 40)
